...
def run_quartile_dummy_regression(
    panel: pd.DataFrame,
    return_col: str = "forward_return_pct",
    reference_quartile: str = "Q4",
    cluster_by: str = "as_of",
    verbose: bool = True,
):
    """Same as run_panel_regression but uses quartile dummies vs Q4 reference.

    Produces explicit Q1-vs-Q4, Q2-vs-Q4, Q3-vs-Q4 coefficients. The Q1
    coefficient is the headline "framework alpha".
    """
    if not HAS_STATSMODELS:
    ...
    model = smf.ols(formula, data=df)
    if cluster_by in df.columns:
        return model.fit(cov_type="cluster", cov_kwds={"groups": df[cluster_by]})
