...
def _normalise_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Bulletproof against SimFin's various index/column layouts:
      - The simfin Python library often returns DataFrames indexed by Ticker
        and/or (Ticker, Report Date) as a MultiIndex. We promote ALL index
        levels back to columns before renaming.
      - Match is case-insensitive on the known SimFin labels — handles
        'Ticker', 'TICKER', 'ticker' equally.
      - Idempotent: safe to call on an already-normalised frame.
    """
    df = df.copy()

    # Step 1: promote ALL named index levels to columns. Handles single-index
    # ('Ticker') and MultiIndex (('Ticker','Report Date')) cases. A frame with
    # only the default integer RangeIndex (index name is None) is left alone.
    if isinstance(df.index, pd.MultiIndex):
        df = df.reset_index()
    elif df.index.name is not None:
        df = df.reset_index()

    # Step 2: case-insensitive rename of known SimFin labels to snake_case.
    canonical = {
        "ticker": "ticker",
        "simfinid": "simfin_id",
        "simfin id": "simfin_id",
    ...
    for col in df.columns:
        ...
        if key in canonical:
            ...
            # Avoid overwriting an existing snake_case column with same target
            if target not in df.columns or target == col:
                rename_map[col] = target
    df = df.rename(columns=rename_map)

    # Step 3: type-coerce date columns
...
def ensure_ticker_column(df: pd.DataFrame, companies: pd.DataFrame) -> pd.DataFrame:
    ...
    # Defensive: if companies hasn't been normalised, try normalising it once.
    # This handles the case where the user passes a fresh simfin DataFrame that
    # still has Ticker as the index or as a capital-case column.
    if not {"simfin_id", "ticker"}.issubset(companies.columns):
        companies = _normalise_columns(companies)

    if not {"simfin_id", "ticker"}.issubset(companies.columns):
        raise KeyError(
            "companies DataFrame must contain both 'simfin_id' and 'ticker'. "
            f"Columns present: {list(companies.columns)[:20]}. "
            "If using sf.load_companies output directly, pass it through "
            "_normalise_columns first, or ensure the Ticker index has been reset."
        )
